...
\begin{tikzpicture}
    ...
    \node [circle, draw=black, inner sep=0pt, minimum size=0.1cm] (v) at (6, 0) {};
    % (6, 0)是圆弧起点坐标，根据起点，半径和起始角度计算圆心绘制圆弧
    ...
    % to: 连接两个点的路径指令，可加参数画曲线 / 弧线; 
    % [out, in] - out：起点的「出射角度」; - in：终点的「入射角度」
...
\newpage
%% 根据函数绘制曲线
% 阶乘函数factorial(\x), 平方根sqrt(\x), 幂函数pow(\x,y), 指数函数exp(\x), 对数函数ln(\x), log10(\x), log2(\x), abs(\x), 取余函数mod(\x,y), 圆整函数round(\x), floor(\x), ceil(\x), sin(\x), cos(\x), tan(\x)等。
% 常数：e=2.718281828, pi=3.141592654
\begin{tikzpicture}[xscale=0.01, yscale=1]
    \draw [<->] (0, 1) -- (0, 0) -- (370, 0);
    \draw [green, thick, domain=0:360] plot (\x, {sin(\x)}); % domain表示自变量取值范围，默认角度; (\x r)表示弧度。数学表达式需要加{}
\end{tikzpicture}
...
\begin{tikzpicture}[xscale=2, yscale=2]
    ...
    \node [left] at (0, 2) {$y$};
    % 标注点 (1, 1)
    \node [below right=1, red] at (1, 1) {below right}; % 参数值 1 表示相对距离
